...
// import { useSuiMove } from '../hooks/useSuiMove';
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, viewType }) => {
    ...
    const handleBuy = () => {
        const amount = parseInt(suiAmount);
        if (!suiAmount || isNaN(amount) || amount <= 0) {
            alert('Please enter a valid, positive amount of SUI.');
            return;
        }
        // // Simulate purchase
        // alert(`Mock purchase successful! You supported "${project.title}" with ${amount} SUI.`);
    };
                    ...
                    {!isArtistView && (
                        <div className="mt-6">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <button
                                    onClick={handleBuy}
                                    className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                >
                                    BUY
                                </button>
                            </div>
